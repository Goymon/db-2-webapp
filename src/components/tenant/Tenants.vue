<template>
    <div class="card">
        <div class="card-header">
            <h4 class="card-title text-center">Tenants</h4>
        </div>
        <div class="card-body">
            ...
            <transition-group name="slide" type="animation" appear mode="out-in" tag="div" class="row">
                <div class="col-md-6 my-4" v-for="(t, index) in filteredT" :key="index">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">{{ t.fname }} {{ t.lname }}</h5>
                            <h6 class="card-subtitle mb-3">Age: {{t.age}}</h6>
                            <h6 class="card-subtitle mb-4">Marital Status: {{t.marital_status}}</h6>
                            <h6 class="card-subtitle mb-4">Occupation: {{t.occupation}}</h6>
...
export default {
    ...
    computed: {
        ...
        pagination() {
            var val = this.Tenant_Total/this.pageQty;
            return  val % 1 == 0 ? val : Math.floor(val) + 1;
        },
        filteredT() {
            return this.tenant.filter((item, index) => {
                var i = index + 1;
                var query;
                if(this.search == '') {
                    ...
                    query = item.fname.toLowerCase().match(this.search.toLowerCase()) ||
                            ...
                            item.age.toString().match(this.search) ||
                            item.marital_status.toLowerCase().match(this.search.toLowerCase()) ||
                            item.occupation.toLowerCase().match(this.search.toLowerCase());
                }
                return query;
            });
        }
